...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((sum, char) => sum + char.charCodeAt(), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((sum, char) => (vowels.includes(char) ? sum + 1 : sum), 0);
}
...
function isPalindrome(str) {
  const onlyLettersString = str.toLowerCase().replace(/[ .,?!]/g, '');
  return onlyLettersString === onlyLettersString.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce(
      (word, nextWord) => (word.length >= nextWord.length ? word : nextWord),
      ''
    );
}
...
function encodeToRot13(str) {
  ...
  const subChars = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((char) => {
      if (chars.indexOf(char) !== -1) {
        return subChars[chars.indexOf(char)];
      }
      return char;
    })
    .join('');
...
function getCardId(value) {
  ...
  const suit = ['♣', '♦', '♥', '♠'];
  const cardIndex = cardValue.indexOf(value.slice(0, -1));
  const suitIndex = suit.indexOf(value.slice(-1)) * cardValue.length;
